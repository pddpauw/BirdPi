...
specie = st.sidebar.selectbox(
    "Which bird would you like to review ?", species, index=idx, key="specie"
)
...
df2 = df2[df2["Com_Name"] == specie]

recordings1 = df2[df2["File_Name"].isin(sound_files)]

# recordings = recordings1[recordings1["Com_Name"] == specie]["File_Name"]

# if 'recording' not in st.session_state:
#     st.session_state['recording'] = recordings[0]

# try:
#     recording = st.selectbox(
#         "Available recordings",
#         recordings.sort_index(ascending=False),
#         help="These are the recordings available on your Birdie-Pi at the moment for you to review",
#     )
#     date_specie = df2.loc[
#         df2["File_Name"] == recording, ["Date", "Com_Name", "Sci_Name"]
#     ]
...
# except:
#     st.write("No recordings unfortunately, try another specie")
...
xc_active = st.sidebar.checkbox("Use Xeno-Canto Comparison?", value=False)


col1, col2 = st.columns((1, 2))

with col2:
    data = recordings1[recordings1["Com_Name"] == specie][
        ["File_Name", "Manual_ID", "Rating", "Confidence"]
...
with col1:
    ...
    with st.form("Review"):
        ...
        st.write("Current ID:", current_ID)
        st.write(
            "Current Rating:",
        ...
        if verified == "False Positive":
            ...
            corrected = st.selectbox(
                "What specie?", df_names["Com_Name"], index=dropdown_index
            )
        else:
        ...
        submitted = st.form_submit_button("Commit to DB")
