...
with open(userDir + "/BirdNET-Pi/scripts/thisrun.txt", "r") as f:
    ...
    lat = str(
        str(str([i for i in this_run if i.startswith("LAT")]).split("=")[1]).split(
            "\\"
        )[0]
    )
    lon = str(
        str(str([i for i in this_run if i.startswith("LON")]).split("=")[1]).split(
            "\\"
        )[0]
    )
...
c = conn.cursor()
c.execute(
    "CREATE TABLE IF NOT EXISTS daylight (date,dawn, sunrise, noon, sunset, dusk)"
)
conn.commit()
